...
#             net, end_points = resnet_v1_WD(x, None, is_training=is_train)
#
#             net = slim.dropout(net, dropout_keep_prob, is_training=is_train, scope='dropout')
...
#     x = x / 128 - 1
#     end_points = {}
#     with tf.variable_scope(name_or_scope='WR28-10', reuse=tf.AUTO_REUSE):
#         global_pool = WD28(x, is_training=is_train)
#         global_pool = tf.nn.dropout(global_pool, dropout_keep_prob)
#         logits = tf.layers.dense(global_pool, 10, name='final_dense')
#     if only_logits:
#         return logits
#     return logits, end_points
...
def tower_fn(is_training, feature, label, open_adv_trainn=False):
    ...
    if open_adv_trainn:
        ...
        bounds = (0, 255)
        adv_feature = generate_adversarial_examples(feature, bounds, model_fn_eval_mode, "pgd_8_2_7")
    ...
    l2_var = [var for var in model_params if 'BatchNorm' not in var.name]
...
def input_fn(data_dir, subset, batch_size):
    with tf.device('/cpu:0'):
        ...
        dataset = cifar.CifarDataSet(data_dir, subset, use_distortion)
